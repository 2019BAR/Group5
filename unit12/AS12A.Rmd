...
lines 為 logit 線性方程式的圖形；

當 X1 , X2 的組合落在 blue 線上，代表預測其 Y = 1 的機率為 0.5。

cyan 線則個別是其他機率下的狀況，就像是機率的等高線圖般。

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>

可以利用各條機率線去計算出該點的機率區間。
...
logit 是 log(P/1-P)，意即對勝率（odd）取對數，為線性架構。

logistic function 是機率（P），為非線性架構。

兩者間的關係可以藉由數學推導（對 logit 取指數，即可得到關係式）互相轉換。

關係式：logit = log(1/1-P)，P = logistic function。
